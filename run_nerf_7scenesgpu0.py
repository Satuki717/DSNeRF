...
def config_parser():
    ...
    parser.add_argument("--dataset_type", type=str, default='llff', 
                        help='options: llff / blender / deepvoxels / 7Scenes')
...
def train():
    ...
    if args.dataset_type == 'colmap_llff':
        ...
        if args.gt_depth:
            depth_gts = make_depths('heads', 'seq-02', '/mnt/datagrid1/yyuan/7scenes', args.datadir, factor=args.factor, bd_factor=None, loadfromfile=args.loaddepth)
    ...
    for i in trange(start, N_iters):
        ...
        if args.gt_depth and not args.depth_with_rgb:
            rgb = rgb[:N_batch, :]
            disp = disp[:N_batch]
            acc = acc[:N_batch]
        ...
        elif args.gt_depth and args.depth_with_rgb:
            depth_col = depth
    ...
    np.save(os.path.join(basedir, 'training_data.txt'), training_data)
